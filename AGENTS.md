...
- `./scripts/update_src.sh jazzy` (or `humble`): import `repos/<distro>.repos` and `repos/<distro>-*.repos`; vendor upstream lives at `*_vendor/*_src/` (CLI: `pybind11_src` in `*-cli.repos` only).
